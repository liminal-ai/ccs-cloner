...
export async function executeCloneOperation(
  ...
  // 1. Find and read source session
  const sourcePath = await findSessionFileById(options.sourceSessionId, claudeDir);
  const { entries: allEntries, rawContent } = await parseSessionFile(sourcePath);
 ...
 */
function updateSessionIds(entries: SessionLineItem[], newSessionId: string): SessionLineItem[] {
 ...
 */
function createSummaryEntry(entries: SessionLineItem[], firstUserMessage: string): SessionLineItem {
