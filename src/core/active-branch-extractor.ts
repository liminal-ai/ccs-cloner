...
export function extractActiveBranchFromSession(
  ...
  if (activeBranchUuids.length === 0) {
    throw new ActiveBranchExtractionError("Could not build active branch chain from leaf to root");
  }
  ...
  return {
    ...
    extractionStatistics: {
      ...
      orphanedEntriesDiscarded: allSessionEntries.length - entriesInActiveBranch,
    },
  };
}
 ...
 */
export function buildUuidGraphFromEntries(entries: SessionLineItem[]): UuidGraph {
 ...
 */
export function walkParentChainToRoot(leafUuid: string, graph: UuidGraph): string[] {
  const chain: string[] = [];
 ...
 */
export function findSummaryLeafUuid(entries: SessionLineItem[]): string | undefined {
  for (const entry of entries) {
