...
export function removeToolCallsFromHistory(
	...
	const removedToolTurns =
		keep > 0 ? turnsWithTools.slice(0, -keep) : turnsWithTools;
	...
	const truncateCount = Math.floor(
		(keptToolTurns.length * options.truncatePercent) / 100,
	);
	...
	const truncatedTurnIndices = new Set(
		truncatedToolTurns.map((t) => t.turnIndex),
	);
			...
			content = content.map((block) => {
				if (block.type === "tool_result" && "content" in block) {
					...
					if (typeof blockContent === "string") {
						...
						const { result: truncatedValue, wasTruncated } =
							truncateObjectValues(blockContent);
						if (wasTruncated) {
							toolCallsTruncated++;
							contentModified = true;
	...
	const finalEntries = modifiedEntries.filter(
		(_, index) => !entriesToDelete.has(index),
	);
...
export function truncateObjectValues(obj: unknown): TruncationResult {
	...
	if (Array.isArray(obj)) {
		...
		const result = obj.map((item) => {
			const { result: truncatedItem, wasTruncated } =
				truncateObjectValues(item);
			if (wasTruncated) {
				anyTruncated = true;
			}
