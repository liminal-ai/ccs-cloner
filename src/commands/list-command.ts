...
import { listAllProjects, listSessionsInProject } from "../io/session-directory-scanner.js";
import { parseSessionFile, extractFirstUserMessage } from "../io/session-file-reader.js";
import { countTurns } from "../core/turn-boundary-calculator.js";
import { formatSessionList, formatError } from "../output/clone-result-formatter.js";
...
async function listSessions(options: ListOptions): Promise<SessionInfo[]> {
  ...
  if (options.project) {
    ...
    filteredProjects = projects.filter(
      (p) =>
        p.path.toLowerCase().includes(searchPath) || p.folder.toLowerCase().includes(searchPath)
    );
  }
