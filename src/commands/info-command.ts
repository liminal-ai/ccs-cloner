...
import { findSessionFileById, decodeProjectPath } from "../io/session-directory-scanner.js";
import {
  parseSessionFile,
...
import { formatSessionDetails, formatError } from "../output/clone-result-formatter.js";
...
export const infoCommand = defineCommand({
  ...
  async run({ args }) {
    try {
      ...
      // Get session details
      const details = await getSessionDetails(args.sessionId, config.claudeDataDirectory);
 ...
 */
async function getSessionDetails(sessionId: string, claudeDir: string): Promise<SessionDetails> {
