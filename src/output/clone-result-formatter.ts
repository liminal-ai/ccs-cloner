...
function formatStatistics(
	stats: CloneOperationStatistics,
	options: OutputFormatOptions,
): string {
	const lines: string[] = [];
	const indent = "  ";
	...
	lines.push(
		`${indent}Turns: ${stats.turnCountOriginal} -> ${stats.turnCountOutput}`,
	);

	if (stats.orphanedEntriesDiscarded > 0 || options.verbose) {
		lines.push(
			`${indent}Orphaned entries discarded: ${stats.orphanedEntriesDiscarded}`,
		);
	}
	...
	if (stats.thinkingBlocksRemoved > 0 || options.verbose) {
		lines.push(
			`${indent}Thinking blocks removed: ${stats.thinkingBlocksRemoved}`,
		);
	}
...
function formatSessionSummary(session: SessionInfo): string {
	...
	lines.push(
		`    Turns: ${session.turnCount} | Size: ${formatBytes(session.sizeBytes)}`,
	);
