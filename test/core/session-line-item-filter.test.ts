...
describe("session-line-item-filter", () => {
	describe("filterCloneableEntries", () => {
		test("filters summary, file-history-snapshot, isApiErrorMessage, and <synthetic> model entries", () => {
			...
			expect(
				result.entries.some((e) => e.type === "file-history-snapshot"),
			).toBe(false);
			expect(result.entries.some((e) => e.isApiErrorMessage === true)).toBe(
				false,
			);
			expect(
				result.entries.some((e) => e.message?.model === "<synthetic>"),
			).toBe(false);
		});
