...
describe("executeCloneOperation (integration)", () => {
	...
	test("clones a session end-to-end without tool removal", async () => {
		...
		expect(result.clonedSessionFilePath).toBe(
			join(PROJECT_DIR, `${result.clonedSessionId}.jsonl`),
		);
		...
		expect((outputEntries[0].summary as string).startsWith("Clone:")).toBe(
			true,
		);
		...
		const entriesWithSessionId = outputEntries.filter(
			(e) => e.sessionId !== undefined,
		);
		...
		expect(outputEntries.some((e) => e.type === "file-history-snapshot")).toBe(
			false,
		);
		expect(outputEntries.some((e) => e.isApiErrorMessage === true)).toBe(false);
		expect(outputEntries.some((e) => e.message?.model === "<synthetic>")).toBe(
			false,
		);
		...
		expect(
			index!.entries.some((e) => e.sessionId === result.clonedSessionId),
		).toBe(true);
	});
